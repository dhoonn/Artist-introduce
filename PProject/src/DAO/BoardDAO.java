...
public class BoardDAO {
	...
	public BoardDTO boardUpdate(int bnumber) {
		...
		try {
			...
			if(rs.next()) {
				...
				boardUpdate.setBcontent(rs.getString("BCONTENT"));
				boardUpdate.setBcategory(rs.getString("BCATEGORY"));
				boardUpdate.setBfile(rs.getString("BFILE"));
				boardUpdate.setBdate(rs.getDate("BDATE"));
				boardUpdate.setBlike(rs.getInt("BLIKE"));
	...
	public int boardUpdateProcess(BoardDTO board) {
		String sql = "UPDATE BOARD SET BWRITER=?, BTITLE=?, BCATEGORY=?, BCONTENT=?, BFILE=? WHERE BNUMBER=?";
		int processResult = 0;
		try {
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, board.getBwriter());
			...
			pstmt.setString(5, board.getBfile());
			pstmt.setInt(6, board.getBnumber());
	...
	public List<BoardDTO> boardSearch(String keyword) {
		String sql = "SELECT * FROM BOARD WHERE BCONTENT LIKE ?";
		List<BoardDTO> boardList = new ArrayList<BoardDTO>();
		BoardDTO board = null;
		try {
			pstmt=con.prepareStatement(sql);
